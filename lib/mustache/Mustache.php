...
class Mustache {
	...
	protected function _setOptions(array $options) {
        ...
        if (isset($options['user_funcs'])) {
            foreach ($options['user_funcs'] as $func_name => $func) {
                if (!is_callable($func)) {
                    throw new InvalidArgumentException('user_funcs must be callable');
                }
            }
            $this->_userFuncs = $options['user_funcs'];
        }
	}
	...
	protected function _renderTag($modifier, $tag_name, $leading, $trailing) {
		switch ($modifier) {
            case '-':
                return $this->_callDefinedFunc($tag_name, $leading, $trailing);
                break;
			case '=':
			...
			case '&':
				if ($this->_hasPragma(self::PRAGMA_UNESCAPED)) {
                    ...
                    $value = $this->_renderTags($value);
                    $rv = $this->_renderEscaped($value, $leading, $trailing);
				} else {
                    $rv = $this->_renderUnescaped($tag_name, $leading, $trailing);
                    $rv = $this->_renderTags($rv);
				}
                return $rv;
				break;
			case '#':
			...
			default:
				if ($this->_hasPragma(self::PRAGMA_UNESCAPED)) {
					$rv = $this->_renderUnescaped($modifier . $tag_name, $leading, $trailing);
                    $rv = $this->_renderTags($rv);
				} else {
                    $value = $this->_renderUnescaped($modifier . $tag_name, '', '');
                    $value = $this->_renderTags($value);
					$rv = $this->_renderEscaped($value, $leading, $trailing);
				}
                return $rv;
	 ...
	 * @return string
	 */
	protected function _renderEscaped($value, $leading, $trailing) {
    ...
    protected function _callDefinedFunc($tag_name, $leading, $trailing) {
        $vals = explode(' ', $tag_name);
        $func_name = array_shift($vals);
        $valsLength = sizeof($vals);
        for($i = 0; $i < $valsLength; $i++) {
            $vals[$i] = $this->_renderTag('', $vals[$i], '', '');
        }
        if(isset($this->_userFuncs[$func_name])) {
            $rendered = call_user_func_array($this->_userFuncs[$func_name], $vals);
            return $leading . $rendered . $trailing;
        }
		return $leading . $trailing;
	}
